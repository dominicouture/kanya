...
class Quantity:
    ...
    def find_precisions(self):
        ...
        self.precisions[zeros] = -np.vectorize(
            lambda x: Decimal(str(x)).as_tuple().exponent, otypes=[np.int]
        )(self.values[zeros])
    ...
    def __mul__(self, other):
        """
        Computes the product for a Quantity. 'other' can be an int or a float, or a np.ndarray
    ...
    def __pow__(self, other):
        """
        Computes the raise to the power for a Quantity object. 'other' can be an integer or a
        float, or np.ndarray or a Quantity object of a shape that can be broadcast to 'self'.
        """
    ...
    def __getitem__(self, index):
        """
        Returns a Quantity object with the specified index.
        !!! Add slicing support !!!
        """
